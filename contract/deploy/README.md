...
- This deployment based on VNT private network on local server/cloud server
- Since ***Dice Contract*** is written into data and logic contract, the data contract need to be deployed first.
...
// Use the following provider if connect to VNT network
...
// the following accounts created on private chain, store locally
...
// these are two parts from compiling this data contract, .compress and .abi
// the path need to adjust accordingly to your output of the compilation.
var codeFile =
...
// for convenience, json files used to store config of each data and logic contract for the develop and deployment of Dice Contract.
...
```
- Test interfaces, external functionalities
  All external functions as well as interfaces should be tested carefullly before the implementation of logic contract.

## Deploy Logic Contract
- after the deployment and testing of functionalities of data contract, we can deploy the logic contract as below
...
// still, these are 04 accounts stored locally
var from1 = '0xf3830b235b2110a177d30e53e313b093f0f0a370';
var from1Keystore = '/home/ubuntu/testnet/node3/keystore/UTC--2020-09-23T11-32-39.249397942Z--88a1493a2eb3358d8d298e53d8afa46609e8a8ab';
var pass1 = 'Test@2020';
  ...
  var contractReturned = contract.new(1000000000000000, "VNT Token", "VNT", {
     ...
     if(!err) {
        if(!myContract.address) {
          ...
          // after successfully deploy the logic contract, the following are used to set:
          async.parallel({
            // 1. set Data contract address on this logic contract, so that it knows how to access its data contract
            ...
            // 2. set logic contract address on data contract so that, data contract can authorise this logic contract
            // for its accessing states on data contract.
...
// a helper function to set data contract from logic contract
...
// a helper function to get data contract address
...
### Testing Logic contract functionalities
